...
import { Loader2, AlertCircle, Sparkles, Mail, Lock, Eye, EyeOff, User, ArrowRight } from 'lucide-react';
...
export default function SignUp() {
  ...
  return (
    <AuthLayout>
      <h1 className="text-white font-black text-3xl sm:text-[34px] tracking-tight">Create your account</h1>
      <p className="flex items-center gap-1.5 text-white/40 text-sm mt-2">
        <Sparkles className="w-3.5 h-3.5 text-[#D1FE17]" />
        Start with 3 free clips — no card needed
      </p>
      ...
      <form onSubmit={submit} className="mt-8 space-y-5">
        <div>
          <label className="block text-[11px] font-black uppercase tracking-widest text-white/40 mb-2">
            Name <span className="text-white/25 normal-case tracking-normal font-semibold">(optional)</span>
          </label>
          <div className="relative">
            <User className="absolute left-4 top-1/2 -translate-y-1/2 w-[18px] h-[18px] text-white/25 pointer-events-none" />
            <input
              type="text"
              autoComplete="name"
              value={name}
        ...
        <div>
          <label className="block text-[11px] font-black uppercase tracking-widest text-white/40 mb-2">Password</label>
          <div className="relative">
      ...
      <p className="text-white/40 text-sm mt-8">
        ...
        <Link href="/login" className="text-[#D1FE17] hover:text-[#c5f010] font-bold">
          Log in
        </Link>
      </p>
    </AuthLayout>
